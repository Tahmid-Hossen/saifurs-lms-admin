            ...
            if (isset($course->batches)):
                foreach ($course->batches->sortBy('course_batch_start_date') as $batch):
                    ...
                    $batch->duration = ucwords(str_replace([' before', ' ago', ' after'], ['', '', ''],
                        \Carbon\Carbon::parse($batch->course_batch_start_date)->diffAsCarbonInterval(\Carbon\Carbon::parse($batch->course_batch_end_date))));
                    ...
                    // batches details
                    $batch->branch;
                    $batch['instructor']['userDetails']['user_detail_photo'] = $batch->instructor->userDetails->user_detail_photo_full_link ?? "https://www.saifurs.com.bd/assets/img/user-default.png";

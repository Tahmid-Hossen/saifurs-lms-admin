...
use App\Http\Requests\Backend\User\StorePasswordRequest;
...
/*
     ...
     public function showChangePasswordGet() {
        return view('backend.user.users.password.create');
    }

    public function showChangePasswordPost(StorePasswordRequest $request) {
        if (!(Hash::check($request->get('current_password'), Auth::user()->password))) {
            // The passwords matches
            flash("Your current password does not matches with the password.")->error();
            return redirect()->back()->withInput();
        }

        if(strcmp($request->get('current_password'), $request->get('new_password')) == 0){
            // Current password and new password same
            flash("New Password cannot be same as your current password.")->error();
            return redirect()->back()->withInput();
        }

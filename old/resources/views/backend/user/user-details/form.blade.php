...
<div class="box-body">
    ...
    <section>
        <div class="row">
            @if(isset($userDetails))
                <input type="hidden" id="user_id" value="{{isset($userDetails) ? $userDetails->user->id: null}}">
            @endif
            <div class="col-md-6">
                {!! \Form::nText('username', 'Username (Unique Identifier)', old('username', isset($userDetails) ? $userDetails->user->username: null) , true) !!}
            </div>
            <div class="col-md-6">
                {!! \Form::nEmail('email', 'Email Address', old('name', isset($userDetails) ? $userDetails->user->email: null) , true) !!}
            </div>
        </div>
        @if(empty($userDetails))
            <div class="row">
                <div class="col-md-6">
                    {!! \Form::nPassword('password', 'Password', (isset($userDetails) ? '' : '123456') , empty($userDetails)) !!}
                    <span class="help-block">
                    {{isset($userDetails) ? 'Left it blank if you do not want to change!!!':'By default: 123456;'}}
                </span>
                </div>
                <div class="col-md-6">
                    {!! \Form::nPassword('confirm_password', 'ReType Password', (isset($userDetails) ? '' : '123456') , empty($userDetails)) !!}
                    <span class="help-block">
                    {{isset($userDetails) ? 'Left it blank if you do not want to change!!!':'By default: 123456;'}}
                </span>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select name="status" id="status" class="form-control">
                        @foreach(\App\Support\Configs\Constants::$user_status as $status)
                            <option value="{{$status}}"
                                    @if (isset($userDetails) && ($userDetails->user->status === $status)) selected @endif
                            >{{$status}}</option>
                        @endforeach
                    </select>
    ...
    <section>
        <div class="row">
            @if(auth()->user()->userDetails->company_id == 1)
                <div class="col-md-6">
                    {!! \Form::nSelect('company_id', 'Company', $global_companies, isset($userDetails) ? $userDetails->company_id:null, true,
        ...
        <div class="row">
            <div class="col-md-6">
                {!! \Form::nText('first_name', 'First Name', isset($userDetails) ? $userDetails->first_name:null, true) !!}
            </div>
            <div class="col-md-6">
                {!! \Form::nText('last_name', 'Last Name', isset($userDetails) ? $userDetails->last_name:null, true) !!}
            </div>
        </div> <!-- end row -->
        <div class="row">
            <div class="col-md-6">
                <div class="form-group @error('gender') has-error @enderror">
                    ...
                    <label class="radio-inline">
                        <input type="radio" name="gender" id="genderMale" value="male"
                               @if(old('gender', isset($userDetails) ? $userDetails->gender:null) == 'male') checked @endif
                        >
                        Male
                    </label>
                    <label class="radio-inline">
                        <input type="radio" name="gender" id="genderFemale" value="female"
                               @if(old('gender', isset($userDetails) ? $userDetails->gender:null) == 'female') checked @endif
                        >
                        Female
                    </label>
                    <label class="radio-inline">
                        <input type="radio" name="gender" id="genderOther" value="other"
                               @if(old('gender', isset($userDetails) ? $userDetails->gender:null) == 'other') checked @endif
                        >
                        Other
                    </label>
            ...
            <div class="col-md-6">
                ...
                isset($userDetails) ? $userDetails->identity_card:null, false,
 ...
 isset($userDetails) ? $userDetails->date_of_birth:null, true,
  ['readonly' => 'readonly', 'placeholder' => 'Enter Birth Date', 'class' => 'form-control input-date']) !!}
            </div>
            <div class="col-md-6">
                {!! \Form::nDate('date_of_enrollment', 'Date of Enrollment',
isset($userDetails) ? $userDetails->date_of_enrollment:null, true,
['readonly' => 'readonly', 'placeholder' => 'Date of Enrollment', 'class' => 'form-control input-date']) !!}
            </div>
        </div>
        <div class="row">
            {{--<div class="col-md-6">
                {!! \Form::nText('national_id', 'National ID', isset($userDetails) ? $userDetails->national_id:null, false) !!}
            </div>--}}
            {{--<div class="col-md-6">
                <div class="form-group">
                    ...
                    <select name="married_status" id="married_status" class="form-control" required>
                        ...
                        @foreach(\Utility::$marriedStatus as $key=>$marriedStatus)
                            <option value="{{ $key }}"
                                    @if(old('married_status', isset($userDetails) ? $userDetails->married_status:null) == $key) selected @endif
...
old('mobile_phone', isset($userDetails) ? $userDetails->mobile_phone: null) , true) !!}
            </div>
            <div class="col-md-6">
                {!! \Form::nTel('home_phone', 'Home Phone',
old('home_phone', isset($userDetails) ? $userDetails->home_phone: null) , false) !!}
            </div>
        </div>
        <div class="row">
            <div class="col-md-12">
        ...
        <div class="row">
            <div class="col-md-12">
                {!! \Form::nTextarea('mailing_address', 'Mailing Address',
                    isset($userDetails) ? $userDetails->mailing_address:null, true, ['rows' => 2]) !!}
            </div>
        </div>
        <div class="row">
            <div class="col-md-4">
            ...
            <div class="col-md-4">
                <div class="form-group">
                    ...
                    <input
                        ...
                        value="{{ old('post_code', isset($userDetails) ? $userDetails->post_code:null) }}"
                    >
            ...
            {{--<div class="col-md-6">
                <div class="form-group">
                    {!! \Form::nLabel('police_station', 'Police Station', true) !!}
                    ...
                    <input
                        ...
                        value="{{ old('police_station', isset($userDetails) ? $userDetails->police_station:null) }}"
                        required
                    >
                    <span id="police_station-error" class="d-block text-danger help-block">
        ...
        <div class="row">
            <div class="col-md-12">
                {!! \Form::nTextarea('shipping_address', 'Shipping Address',
                    isset($userDetails) ? $userDetails->shipping_address:null, false, ['rows' => 2]) !!}
            </div>
        </div>
        <div class="row">
            <div class="col-md-4">
            ...
            <div class="col-md-4">
                <div class="form-group">
                    ...
                    <input
                        ...
                        placeholder="Enter post code"
                        value="{{ old('shipping_post_code', isset($userDetails) ? $userDetails->shipping_post_code:null) }}"
                    >
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <input
                                ...
                                value="{{ old('user_detail_photo', isset($userDetails) ? $userDetails->user_detail_photo:null) }}"
                            >
                        ...
                        <div class="form-group">
                            ...
                            <label class="radio-inline">
                                <input type="radio" name="user_details_verified"
                                       id="userDetailsVerifiedEnable"
                                       value="{{\App\Support\Configs\Constants::$user_active_status}}"
                                       @if(old('user_details_verified', isset($userDetails) ? ($userDetails->user_details_verified):null) == \App\Support\Configs\Constants::$user_active_status) checked @endif
                                >
                                Yes
                            </label>
                            <label class="radio-inline">
                                <input type="radio" name="user_details_verified"
                                       id="userDetailsVerifiedDisable"
                                       value="{{\App\Support\Configs\Constants::$user_default_status}}"
                                       @if(old('user_details_verified', isset($userDetails) ? ($userDetails->user_details_verified):null) == \App\Support\Configs\Constants::$user_default_status) checked @endif
                                >
                                No
                            </label>
...
@push('scripts')
    <script>
        const selected_state_id = '{{old("state_id", (isset($userDetails)?$userDetails->state_id:null))}}';
        const selected_shipping_state_id = '{{old("shipping_state_id", (isset($userDetails)?$userDetails->shipping_state_id:null))}}';
        const selected_city_id = '{{old("city_id", (isset($userDetails)?$userDetails->city_id:null))}}';
        const selected_shipping_city_id = '{{old("shipping_city_id", (isset($userDetails)?$userDetails->shipping_city_id:null))}}';
        const selected_branch_id = '{{old("branch_id", (isset($userDetails)?$userDetails->branch_id:null))}}';
        const selected_company_id = '{{old("company_id", (isset($userDetails)?$userDetails->company_id:null))}}';

        $(document).ready(function () {

            formObject = $('form#user_details_form');
            ...
            formValidator = formObject.validate({
                rules: {
                    ...
                    password: {
                        required: {{ isset($userDetails) ? 'false' : 'true' }},
                        minlength: 6,
                        maxlength: 255,

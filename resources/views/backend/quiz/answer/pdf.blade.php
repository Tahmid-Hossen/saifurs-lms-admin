    ...
    $html .= '<table width="100%" style="padding: 2px; font-size:12px;" cellpadding="0" cellspacing="0" border="1">';
        $html .= '<thead>';
            $html .= '<tr>';
                ...
                $html .= '<th align="center"><b>Answer</b></th>';
    ...
    if(isset($question_answers) && !empty($question_answers)):
        foreach($question_answers as $answer):
            ...
            $html .= '<tbody>';
                $html .= '<tr>';
                    $html .= '<td>'.$count.'</td>';
                    $html .= '<td>'.(isset($answer) ? strtoupper($answer->courseQuestion->question):null).'</td>';
                    $html .= '<td>'.strtoupper($answer->answer).'</td>';
                    $html .= '<td align="center">'.str_replace("-","",$answer->answer_status).'</td>';
                    $html .= '<td>'.\Carbon\Carbon::parse($answer->created_at)->format('M d, Y h:i:s A').'</td>';
                $html .= '</tr>';
            $html .= '</tbody>';
        endforeach;
    endif;
...
*/    $mpdf->SetHTMLHeader('
        <table width="100%">
            ...
            <tr><td align="center"><b>Answer List For '.date('F Y').'</b></td></tr>
        </table><br><br>
    ');
    $mpdf->AddPage('L', 'A4');
    ...
    ');
    $mpdf->WriteHTML(($html));
    $mpdf->Output('Answer Report For '.date('F Y').'.pdf', 'D');
@endphp

...
class BookController extends Controller
{
	...
	public function stockHistory(Request $request)
    {
        ...
        $books = $this->bookService->getBookInventory($filters)->paginate($request->display_item_per_page);
		//dd($books);
        return view('backend.book.book.stock-history', [
            'books' => $books,
            'inputs' => $request->query(),
            'filters' => $filters,
        ]);
	...
	public function updateStocks(Request $request){
		...
		if(isset($request->bookid) && $request->bookid!="")
         {
            $bookdata = $this->bookService->updateStocks($request->all());
			flash('Stock update successfully', 'success');
            return redirect()->route('books.index');
         }
		 else{
		 	 flash('Failed to stock update!', 'error');
    ...
    public function create()
    {
        return view('backend.book.book.create', [
            'categories' => $this->categoryService->getCategoryDropDown(),
            'countries' => $this->countryService->getCountryDropDown(),
    ...
    public function store(BookRequest $bookRequest): RedirectResponse
    {
        ...
        $pdf_file = $this->bookService->getUploadedPDFFile($bookRequest);
            if (!empty($image_path) && $ebook_file !== false && $pdf_file !== false) {
                \DB::beginTransaction();
                try {
                    $book = $this->bookService->getRequestDataFormatted($bookRequest->except('_token'), $image_path, $ebook_file, $pdf_file);
    ...
    public function update(BookRequest $bookRequest, int $id): RedirectResponse
    {
        ...
        $pdf_file = $this->bookService->getUploadedPDFFile($bookRequest);
        $ebook_file = $this->bookService->getUploadedFile($bookRequest);
        if (!empty($image_path) && $pdf_file !== false) {
            try {
                $book = $this->bookService->getRequestDataFormatted($bookRequest->except('_token'), $image_path, $ebook_file, $pdf_file);
                $keywords = $this->keywordService->getAllKeywordId($bookRequest->input('book_keywords.*'));

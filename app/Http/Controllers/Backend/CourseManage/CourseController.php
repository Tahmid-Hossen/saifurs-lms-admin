...
class CourseController extends Controller
{
    ...
    public function create()
    {
        try {
			...
			$branches = Branch::where('company_id',auth()->user()->userDetails->company_id)->get();

            return view('backend.course.course.create', compact('companies', 'tags', 'course_categories', 'course_sub_categories', 'course_child_categories','branches'));
        } catch (\Exception $e) {
    ...
    public function crop(Request $request){
        $path = UtilityService::$imageUploadPath['course_image'];
        $file = $request->file('cropableimage');
        //$new_image_name = 'UIMG'.date('Ymd').uniqid().'.jpg';
        $new_image_name = 'course_image_' . uniqid() . "_" . date('Y_m_d_h_i_s').'.jpg';;
        $upload = $file->move(public_path($path), $new_image_name);
        if($upload){
            return response()->json(['status'=>1, 'msg'=>'Image has been cropped successfully.', 'croppedimagename'=>$new_image_name]);
        }else{
...
//		dd($request->all());
    ...
    public function show($id)
    {
        ...
        try {
            ...
            $imagepath = UtilityService::$imageUploadPath['course_image'];
            return view('backend.course.course.show', compact('course','imagepath'));
        } catch (\Exception $e) {
            \Log::error($e->getMessage());
    ...
    public function edit($id)
    {
        try {
            ...
            $selectedbranches = Branch::whereIn('id', explode(',' , $course->branch_id))->get();

            return view('backend.course.course.edit', compact('branches','course', 'tags', 'companies', 'course_categories', 'course_sub_categories', 'course_child_categories','imagepath','selectedbranches'));
        } catch (\Exception $e) {
            \Log::error($e->getMessage());
            flash('Course data not found!')->error();
            return redirect()->route('course.index');

...
class Quiz extends BaseController
{
    ...
    public function remove()
    {
        ...
        $json_arr['message'] = "Quiz removed successfully";
    ...
    public function getList()
    {
        ...
        if ($search == '') {
            ...
            $query = $db2->query("select sq_quiz.*, course.course_title from sq_quiz inner join course on sq_quiz.course_id = course.id where  sq_quiz.trash_status='0'  $where order by sq_quiz.id desc limit $limit, $maxRowsPerPage");
        } else {
            $query = $db2->query("select sq_quiz.*, course.course_title from sq_quiz inner join course on sq_quiz.course_id = course.id where  sq_quiz.trash_status='0' and sq_quiz.quiz_name like '%$search%'  order by sq_quiz.id desc limit $limit, $maxRowsPerPage");
    ...
    public function getMyList()
    {
        ...
        $query = $db2->query("select id, group_ids from users where  user_token='$user_token' ");
        ...
        if ($search == '') {
            ...
            $sql = "select sq_quiz.*, course.course_title from sq_quiz inner join course on sq_quiz.course_id = course.id where  sq_quiz.trash_status='0' and FIND_IN_SET($group_ids,gids) $where order by sq_quiz.id desc limit $limit, $maxRowsPerPage";
            $query = $db2->query($sql);
        } else {
            $sql = "select sq_quiz.*, course.course_title from sq_quiz inner join course on sq_quiz.course_id = course.id where  sq_quiz.trash_status='0' and FIND_IN_SET($group_ids,gids) and sq_quiz.quiz_name like '%$search%'  order by sq_quiz.id desc limit $limit, $maxRowsPerPage";
    ...
    public function add()
    {
        ...
        if ($db1->table('sq_quiz')->insert($userdata)) {
            ...
            $json_arr['status'] = "success";
            $json_arr['id'] = $id;
            $json_arr['message'] = "Quiz added successfully";
            return json_encode($json_arr);
    ...
    public function changeQidsOrder()
    {
        ...
        $json_arr['status'] = "success";
        $json_arr['message'] = "Order updated successfully";
        return json_encode($json_arr);
    ...
    public function validateQuiz()
    {
        ...
        if (count($query->getResultArray()) == 0) {
            ...
            if (!in_array($group_ids, $assigned_gids)) {
                $json_arr['status'] = "failed";
                $json_arr['message'] = "Quiz is not assigned to your group";
                return json_encode($json_arr);
            }
            if ($end_datetime < time()) {
                $json_arr['status'] = "failed";
                $json_arr['message'] = "Quiz has been ended. Please check end date-time";
                return json_encode($json_arr);
            }
            if ($start_datetime > time()) {
                $json_arr['status'] = "failed";
                $json_arr['message'] = "Quiz is not available yet. Please check start date-time";
                return json_encode($json_arr);
            }
            if (count($query2->getResultArray()) >= $max_attempt) {
                $json_arr['status'] = "failed";
                $json_arr['message'] = "You have reached maximum attempt available (" . count($query2->getResultArray()) . "/" . $max_attempt . ") for this quiz";
                return json_encode($json_arr);
            }
            ...
            if ($db1->table('sq_result')->insert($userdata)) {
                ...
                $json_arr['rid'] = $rid;
                $json_arr['message'] = "";
                $json_arr['maximum_datetime'] = $maximum_datetime;
    ...
    public function submitQuiz($rid, $rby = 'System')
    {
        ...
        if ($rby != "System") {
            $json_arr['status'] = "success";
            $json_arr['rid'] = $rid;
            $json_arr['message'] = "Quiz submitted successfully";
            return json_encode($json_arr);
    ...
    public function getQuestions()
    {
        ...
        $query = $db2->query("select id, question_type, question, category_ids, audio from sq_question where id in ($assigned_qids) and trash_status='0' ORDER BY FIELD(id, $assigned_qids) ");
        $result = $query->getResultArray();
        $questions = $result;
    ...
    public function saveAnswer()
    {
        ...
        $rid = $this->request->getVar('rid');
        $query = $db2->query("select id from users where  user_token='$user_token' ");
        $user = $query->getRowArray();
        $uid = $user['id'];
    ...
    public function updateScore()
    {
        ...
        $query = $db2->query("select sq_result.*, sq_quiz.quiz_name, sq_quiz.duration, sq_quiz.correct_score, sq_quiz.incorrect_score, users.username,   users.email,   users.name from sq_result join  sq_quiz on sq_quiz.id=sq_result.quid join users on users.id=sq_result.uid where  sq_result.trash_status='0' and sq_result.result_status !='Open'  $where  ");
        ...
        $json_arr['status'] = "success";
        $json_arr['message'] = "Score updated successfully";
        return json_encode($json_arr);
    ...
    public function getCourses()
    {
        ...
        $company_id = $this->loggedUserCompanyId($user_token);
        $query = $db2->query("select * from course where course_status = 'ACTIVE' AND company_id = $company_id");

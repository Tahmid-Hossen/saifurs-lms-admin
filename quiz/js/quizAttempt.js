                    ...
                    $.post(api_site_url + "quiz/getQuestions", arg, function (data) {
                            ...
                            if (result.status == "success") {
                                ...
                                $(result.data).each(function (index, value) {
                                        ...
                                        //player setting
                                        if (value.question.audio != null) {

                                            if (value.question.audio.length > 0) {
                                                var playerBlock = '<audio controls style="width: 100%" class="audio-player" id="audio-player-' + index + '" >\n' +
                                                    '<source src="' + value.question.audio + '">\n' +
                                                    'Your browser does not support the audio element.\n' +
                                                    '</audio>';

                                                questionData = questionData.replace("AUDIO_PLAYER", playerBlock);

                                            } else {
                                                questionData = questionData.replace("AUDIO_PLAYER", "");
                                            }
                                        } else {
                                            questionData = questionData.replace("AUDIO_PLAYER", "");
                                        }
...
function updateOnlineStatus() {
    ...
    $.ajax({
        ...
        error: function (jqXHR) {
            if (jqXHR.status == 0) {
                $('#connectionStatus').html("Internet disconnected <a href='javascript:updateOnlineStatus();' style='color:#fdfafa;' ><i class='fa fa-sync-alt'></i></a>");
                $('#connectionStatus').removeClass('connected');
                $('#connectionStatus').addClass('notConnected');

            }
        },
        success: function () {
            $('#connectionStatus').html("Connected with internet");
            $('#connectionStatus').addClass('connected');
            $('#connectionStatus').removeClass('notConnected');

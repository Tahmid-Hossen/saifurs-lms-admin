        ...
        <div class="col-xs-12">
            <div class="box box-default collapsed-box">
                <div class="box-header with-border">
                    <h3 class="box-title">Find Result</h3>
                ...
                <!-- /.box-header -->
                <div class="box-body">
                    <form class="horizontal-form" role="form" method="GET" action="{{ route('results.index') }}">
                        <div class="row">
                            <div class="col-md-4">
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered table-hover table-checkable order-column"
                            ...
                            @foreach($results as $result)
                                ...
                                <tr class="odd gradeX">
                                    <td style="color: #76767f">{{isset($result->company)?$result->company->company_name:null}}</td>
                                    <td style="color: #76767f">{{isset($result->course)?$result->course->course_title:null}}</td>
                                <!-- <td style="color: #76767f">{{isset($result->courseBatch)?$result->courseBatch->course_batch_name:null}}</td> -->
                                <!-- <td style="color: #76767f">{{isset($result->courseClass)?$result->courseClass->class_name:null}}</td> -->
                                    <td style="color: #76767f">{{isset($result->user)?$result->user->name:null}}</td>
                                    <td style="color: #76767f">{{isset($result->quiz)?$result->quiz->quiz_topic:null}}</td>
                                    <td>{{isset($result->total_score)?$result->total_score:null}}</td>
                                <!-- <td>
                                    @if($result->result_status == 'IN-ACTIVE')
                                    ...
                                    <td>
                                        {!! \CHTML::flagChangeButton($result,
                                                'result_status',
                                            ['on' => 'ACTIVE', 'off' => 'INACTIVE']) !!}

                                    </td>
                                    <td> {{isset($result->created_at)?$result->created_at->format('d M, Y'):null}}</td>
                                   <td class="tbl-action">
                                        {!!
                                            \CHTML::actionButton(
                                                $reportTitle='..',
                                                $routeLink='#',
                                                $result->id,
                                                $selectButton=['showButton','deleteButton'],

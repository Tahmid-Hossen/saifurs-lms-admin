                    ...
                    <table class="table table-striped table-bordered table-hover table-checkable order-column" id="sample_1"
                        ...
                        <thead>
                            <tr>
                               ...
                               <th><sup class="text-success">Pass Mark</sup> </th>
                                <th>Course Title</th>
								<th class="text-center">Re-Attempt</th>
                                <th>Status</th>
                                <?php /*?><th class="tbl-action">Actions</th><?php */?>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($datas as $data)
                            	@php 
                                    ...
                                    $courses = App\Models\Backend\Course\Course::whereIn('id',$course_id)->get(); 
                                @endphp
                                <tr class="odd gradeX">
                                    <td>{{ $datas->firstItem() + $loop->index }}</td>
                                    <td>{{ $data->id }} | <a href="{{ route('quizzes.show', $data->id) }}">{{ $data->quiz_topic }}</a></td>
                                    <td>{{ $data->quiz_type }}</td>
									<td><a href="{{ $data->quiz_url }}">{{ $data->quiz_url }}</a></td>
                                    <td class="text-center">{{ $data->quiz_full_marks }}</td>
                                    <td>{{ $data->quiz_pass_percentage }}</td>
                                    <td>
                                        @if(count($courses) > 0)
                                        	@foreach($courses as $course)
                                        		<a href="{{ route('course.show', $course->id) }}" target="_blank">{{ $course->course_title }}, </a>
                                            @endforeach
                                        @endif
                                    </td>
                                    <td>{{ $data->quiz_re_attempt }}</td>
                                    <td>{!! \CHTML::flagChangeButton($data, 'quiz_status', \Utility::$statusText) !!}</td>
                                    <?php /*?><td>

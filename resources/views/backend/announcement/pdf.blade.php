...
if (isset($datas) && !empty($datas)):
    foreach ($datas as $data):
        ...
        $html .= '<tr>';
        $html .= '<td>' . $count . '</td>';
        $html .= '<td>' . ($data->announcement_title ?? null) . '</td>';
        $html .= '<td>' . ($data->course->course_title ?? null) . '</td>';
        $html .= '<td>' . ($data->announcement_status ?? null) . '</td>';
        $html .= '<td>' . \Carbon\Carbon::parse($data->created_at)->format(config('app.date_format2')). '</td>';
        $html .= '</tr>';
        $html .= '</tbody>';
    endforeach;
endif;
...
$mpdf->SetHTMLHeader(
    '
        ...
        config('app.address') .
        '</b></td></tr>
                <tr><td align="center"><b>Announcements Report For ' .
        date('F Y') .
        '</b></td></tr>
            </table><br><br>
        ',
...
$mpdf->SetHTMLFooter('
        ...
        ');
$mpdf->WriteHTML($html);
$mpdf->Output('Announcements Report For ' . date('F Y') . '.pdf', 'D');
@endphp

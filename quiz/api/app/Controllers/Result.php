...
class Result extends BaseController
{
    ...
    public function getList()
    {
        ...
        if ($search == '') {
            ...
            $query = $db2->query("select sq_result.*, sq_quiz.quiz_name, users.username,   users.email,   users.name as full_name, course.course_title from sq_result join  sq_quiz on sq_quiz.id=sq_result.quid join users on users.id=sq_result.uid join course on sq_quiz.course_id = course.id where  sq_result.trash_status='0' and sq_result.result_status !='Open'  $where order by sq_result.id desc limit $limit, $maxRowsPerPage");
        } else {
            $query = $db2->query("select sq_result.*, sq_quiz.quiz_name, users.username,   users.email,   users.name as fullname, course.course_title from sq_result join  sq_quiz on sq_quiz.id=sq_result.quid join users on users.id=sq_result.uid join course on sq_quiz.course_id = course.id where  sq_result.trash_status='0' and sq_result.result_status !='Open'  and (sq_quiz.quiz_name like '%$search%' or sq_result.id like '%$search%'  or users.username like '%$search%'   or users.email like '%$search%'     or users.name like '%$search%'  ) order by sq_result.id desc limit $limit, $maxRowsPerPage");
    ...
    public function getMyList()
    {
        ...
        if ($search == '') {
            ...
            $query = $db2->query("select sq_result.*, sq_quiz.quiz_name, users.username,   users.email,   users.name as full_name, course.course_title from sq_result join  sq_quiz on sq_quiz.id=sq_result.quid join users on users.id=sq_result.uid  join course on sq_quiz.course_id = course.id where  sq_result.trash_status='0' and sq_result.result_status !='Open' and sq_result.uid='$uid'  and (sq_quiz.quiz_name like '%$search%' or sq_result.id like '%$search%'  or users.username like '%$search%'   or users.email like '%$search%'     or users.name like '%$search%'  ) order by sq_result.id desc limit $limit, $maxRowsPerPage");
    ...
    public function downloadReport()
    {
        ...
        $where = "";
        if ($group_id != "0") {
            $where = " and  users.group_ids='" . $group_id . "' ";
        }
        if ($from != 0) {
        ...
        $query = $db2->query("select sq_result.*, sq_quiz.quiz_name, users.username,   users.email, users.group_ids,   users.name as full_name, course.course_title from sq_result join  sq_quiz on sq_quiz.id=sq_result.quid join users on users.id=sq_result.uid join course on sq_quiz.course_id=course.id where  sq_result.trash_status='0' and sq_result.result_status !='Open'  $where order by sq_result.id desc ");
        $result = $query->getResultArray();
        $csvData = "Result ID,Username,Email,Full Name, Course, Quiz Name, Obtained Marks, Obtained Percentage, Result, Attempted Time, Total Time Spent, Total Questions, Attempted Questions, Correct Answers, Incorrect Answers";
        foreach ($result as $k => $row) {
            ...
            $csvData = $csvData . "" . $row['id'] . "," . $row['username'] . "," . $row['email'] . "," . $row['full_name'] . "," . $row['course_title'] . "," . $row['quiz_name'] . "," . $row['obtained_score'] . "," . $row['obtained_percentage'] . "," . $row['result_status'] . "," . date('Y-m-d H:i:s', $row['attempted_datetime']) . "," . gmdate("H:i:s", $row['time_spent']) . "," . count(explode(',', ($row['assigned_qids']))) . "," . $attempted_questions . "," . $correct . "," . $incorrect;
        }
        // Type#3 - Write file inside /public folder and return value
        $fill = time();
        $filename = WRITEPATH . $fill . ".csv";
        if (!write_file($filename, $csvData)) {
            $json_arr['status'] = "failed";
            $json_arr['message'] = "Unable to write file on server";
        } else {
            $json_arr['status'] = "success";
            $json_arr['path'] = $fill;
        }
    ...
    public function downloadFile($file): ?DownloadResponse
    {
        return $this->response->download('QUIZ-ATTEMPT-REPORT-' . date('YMD') . '.csv',
            file_get_contents(WRITEPATH . $file . ".csv"));
    }

    public function makeRequiredArrayFormat($arr): array
    {

...
class Login extends BaseController
{
    ...
    public function index()
    {
        ...
        $db = Database::connect();
        $query = $db->query("select users.*, roles.id AS account_type_id from users
        ...
        if ((isset($row['username']) && $row['username'] == $username) && password_verify($password, $row['password'])) {
			...
			$sqlupdate = "update users set user_token='".$user_token."' where id=$uid";
            $updatetoken = $db1->query($sqlupdate);

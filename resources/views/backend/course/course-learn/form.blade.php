    ...
    <div class="row">
        <div class="col-md-12">
            <div class="row">
                <div class="col-md-12">
                    <div class="form-group">
                        {!! \Form::nLabel('learn_status', 'Status', false) !!}
                        <select name="learn_status" id="learn_status" class="form-control view-color">
                            @foreach(\App\Support\Configs\Constants::$course_learn as $status)
                                <option value="{{$status}}"
                                        @if (isset($course_learn) && ($course_learn->learn_status === $status)) selected @endif
                                >{{str_replace("-","",$status)}}</option>
                            @endforeach
                        </select>
                        <span class="form-text text-danger" role="alert">{{ $errors->first('learn_status') }}</span>
                    </div>
                </div>

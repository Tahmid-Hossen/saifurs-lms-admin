...
//        $query->leftJoin('branches', 'branches.id', '=', 'course.branch_id');
        $query->leftJoin('branches', \DB::raw("FIND_IN_SET(branches.id, course.branch_id)"),">",\DB::raw("'0'"));
...
//WHERE course_chapters.deleted_at IS NULL AND course_classes.deleted_at IS NULL
        ...
        $select[] = \DB::raw('IFNULL(IF(course.course_image REGEXP "https?", course.course_image, CONCAT("'.url('/public/upload/images/course_image/').'/",course.course_image)), CONCAT("'.url('/').'","/assets/img/default.png")) AS course_image');
        ...
        $query->select($select)->distinct('course.id');
...
//            $query->where('course.course_status', '=', $filters['course_status']);
//        }
//        if (isset($filters['course_language']) && $filters['course_language']) {
//            $query->where('course.course_language', '=', $filters['course_language']);
//        }
//
//        if (isset($filters['course_featured']) && $filters['course_featured']) {
//            $query->where('course.course_featured', '=', $filters['course_featured']);

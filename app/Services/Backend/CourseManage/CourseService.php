...
class CourseService
{
    ...
    {
        try {
            $course_data = $request->except(['course_file', 'course_video', '_token', 'course_tags','course_type']);
            if ($request->course_option == 'Online'){
                $course_data['course_type'] = join(',' , $request['course_type']);
            }
            if ($request->course_option == 'Offline'){
                $course_data['branch_id'] = join(',' , $request['branch_id']);
            }
...
//            $course_data = $request->except(['course_image', 'course_file', 'course_video', '_token', 'course_tags']);
            $course_data = $request->except(['course_file', 'course_video', '_token', 'course_tags']);
            if ($request->course_option == 'Online'){
                $course_data['course_type'] = join(',' , $request['course_type']);
            }
            if ($request->course_option == 'Offline'){
